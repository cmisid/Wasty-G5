...
"""
@author: Groupe[5]
fichier: score.py version 1.0.0
"""
...
'''
def timeBetweenNodes(time, arrival_node, duration):
    ...
    if arrival_node[1] != None and (time +
        datetime.timedelta(minutes = duration + RETRIEVAL_TIME) < arrival_node[1]):
        ...
        if arrival_node[2] != None and (time +
            datetime.timedelta(minutes = duration + RETRIEVAL_TIME)) > arrival_node[2]:
            return 100000
...
'''
def calculatePath(nodes, mode_transport, gmaps):
  path = []
  for i in nodes:
      for j in nodes:
          if (j != i) :
...
'''
def shortest_path(nodes, aplha, beta, departure_time):
    score = 0
    time = 0
    has_best_path = False
    result = []
    # Recuperatioon de l'API googlemap
    ...
    path_list = calculatePath(nodes, "driving", gmaps)
    ...
    for nb_nodes in range(2,len(nodes) + 1):
        ...
        for subset in itertools.permutations(nodes, nb_nodes):
            if(subset[0] == nodes[0]):
                ...
                # et on clcule la somme des scores de tous les trajets.
                ...
                while (time < 100000) and i < (len(subset) - 1):
                    ...
                    while (time < 100000) and j <(len(path_list)):
                        if path_list[j][0] == subset[i] and path_list[j][1] == subset[i + 1]:
                            time = timeBetweenNodes(departure_time, subset[i + 1] , path_list[j][3])
